...
import com.road.check.report.module.AlongLine.SelectedErrorAdapter;
import com.road.check.report.module.Roadbed.PhotoAdapter;
...
public class Bed {
	...
	public Bed(View view,Activity activity,DatabaseService dbs){
	...
	public void init(Round roundItem,String direction){
		...
		selected_error_listview_bed = (ListView)baseView.findViewById(R.id.selected_error_listview);
	...
	class SelectedErrorAdapter extends ArrayAdapter<Disease>{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (item.content.equals("路肩边沟不洁")) {
				error_degree_list.clear();
				error_degree_list.add("");
				error_degree_adapter.notifyDataSetChanged();
			}
			if (item.content.equals("路肩损坏")) {
				error_degree_list.clear();
				error_degree_list.add("轻");
				error_degree_list.add("重");
				error_degree_adapter.notifyDataSetChanged();
			}
			if (item.content.equals("边坡坍塌")) {
				error_degree_list.clear();
				error_degree_list.add("轻");
				error_degree_list.add("中");
				error_degree_list.add("重");
			...
			if (item.content.equals("水毁冲沟")) {
				error_degree_list.clear();
				error_degree_list.add("轻");
				error_degree_list.add("重");
				error_degree_adapter.notifyDataSetChanged();
			}
			if (item.content.equals("路基构造物损坏")) {
				error_degree_list.clear();
				error_degree_list.add("轻");
				error_degree_list.add("中");
				error_degree_list.add("重");
			...
			if (item.content.equals("路缘石破损")) {
				error_degree_list.clear();
				error_degree_list.add("");
				error_degree_adapter.notifyDataSetChanged();
			}
			if (item.content.equals("排水系统淤塞")) {
				error_degree_list.clear();
				error_degree_list.add("轻");
				error_degree_list.add("重");
				error_degree_adapter.notifyDataSetChanged();
    ...
    public class PhotoImgButtonOnClickListener implements OnClickListener{
		...
		public void onClick(View v) {
			...
			try {
				...
				baseActivity.startActivityForResult(intent, BED_TAKEPHOTO);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
    ...
    public String savaDatas(){
    	if (imgFilePathList.size()==0) {
			Toast.makeText(baseActivity, "未进行拍照，请先拍照再填写数据后保存", 0).show();
			return "4";
		}
    	if((!selectedContentList.get(0).content.equals("路肩边沟不洁")&&!selectedContentList.get(0).content.equals("路缘石缺损"))&& sp_degree.getSelectedItem().toString().equals("")){
    		return "1";
    	}
